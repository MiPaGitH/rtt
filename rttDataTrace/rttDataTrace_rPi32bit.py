...
def getSamples():
  ...
  c=[]
  for i in range(8, len(existingshm.buf)-4, 4):
    c.extend([int.from_bytes(existingshm.buf[i:i+4], byteorder='little')])

  # print('c:',c)

  if ((cAdrInt != 0) and (True == samplingStatus) ):
    head = existingshm.buf[4]
    tail = existingshm.buf[5]
    if ( head > tail):
